...
app = FastAPI(
    ...
    description="Production inference API for department routing, sentiment analysis, and urgency scoring.",
)
...
def root():
    return {
        "message": "AI Grievance Intelligence System API",
        "status": "running",
        "version": MODEL_VERSION,
        "docs": "/docs",
        "health": "/health",
...
def predict(request: PredictionRequest):
    ...
    try:
        ...
        department_confidence = float(
            department_predictor.predict_proba([text])[0].max()
        )
